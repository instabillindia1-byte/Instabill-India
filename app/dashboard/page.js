...
function calcMonthly(invoices) {
  ...
  invoices.forEach((inv) => {
    ...
    if (!map[key]) {
      map[key] = { key, label: lbl, total: 0, paid: 0, pending: 0, gst: 0, count: 0 };
    }
    const total = parseFloat(inv.total_amount) || 0;
    const base = parseFloat(inv.amount) || 0;
    map[key].total += total;
    map[key].gst += total - base;
    map[key].count++;
    if (inv.status === "paid") map[key].paid += total;
...
function calcGST(invoices) {
  ...
  invoices.forEach((inv) => {
    const base = parseFloat(inv.amount) || 0;
    const rate = parseFloat(inv.gst_rate) || 18;
...
function calcClients(invoices) {
  ...
  invoices.forEach((inv) => {
    if (!map[inv.client_name]) {
      map[inv.client_name] = { name: inv.client_name, total: 0, count: 0 };
    }
    map[inv.client_name].total += parseFloat(inv.total_amount) || 0;
...
function DonutChart({ cgst, sgst, igst }) {
  ...
  const segs = [
    { label: "CGST", value: cgst, color: "#0EA5E9" },
    { label: "SGST", value: sgst, color: "#38BDF8" },
    { label: "IGST", value: igst, color: "#7C3AED" },
  ].filter((s) => s.value > 0);
            ...
            <div className="dash-box" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12, marginBottom: 16 }}>
              {[
                ...
                <div key={s.label} className="stat-box" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 16, padding: "20px 18px", animation: `fadeUp 0.5s ease ${idx * 80}ms both` }}>
                  <div style={{ display: "flex", alignItems: "center", justifyValue: "space-between", marginBottom: 12, justifyContent: "space-between" }}>
